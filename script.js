...
function render() {
  ...
  filteredSongs.forEach((s, i) => {
    ...
    // Add playing animation if this is the currently playing song
    if (i === currentIndex && !audio.paused) {
      ...
      for (let j = 0; j < 4; j++) {
        ...
        playingAnimation.appendChild(bar);
      }
      songTitle.appendChild(playingAnimation);
    }
...
playBtn.onclick = () => {
  ...
  if (audio.paused) {
    ...
    `;
    render(); // Update playing animation
  } else {
    audio.pause();
    playBtn.innerHTML = `
      <svg viewBox="0 0 16 16" width="16" height="16">
